 ...
 */
package ro.seava.j4e.presenter.action.query;
...
public class QueryBuilderWithJpql<M, F, P> extends
	...
	private void attachSortClause(StringBuffer eql) {
		if (this.sort != null) {
			...
			if (defaultSort != null && !defaultSort.equals("")) {
				if (logger.isDebugEnabled()) {
					logger.debug("Attaching default order by: " + defaultSort);
				}
	...
	protected void addFetchGroup(Query q) {
		...
		boolean disableFecthGroups = this.getSettings().getAsBoolean(
				Constants.PROP_DISABLE_FETCH_GROUPS);
